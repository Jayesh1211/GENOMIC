...
Original file is located at
    https://colab.research.google.com/drive/1m2RVimffisXS2lZyIFW4pXJn94KLYwVx
"""
    ...
    if st.button("Start Training"):
        ...
        with st.spinner("Training in progress..."):
            ...
            for epoch in range(num_epochs):
                ...
                # [Previous epoch training code remains the same...]

            # After training is complete, create visualization plots
            with plots_container:
                st.header("Training Visualization")

                # Create train scores plot
                fig_train, ax_train = plt.subplots(figsize=(10, 6))
                for idx, client in enumerate(clients):
                    ax_train.plot(client.train_scores, label=f'Client {idx + 1}')
                ax_train.set_xlabel('Epochs')
                ax_train.set_ylabel('Train Score')
                ax_train.set_title(f"Train Scores for All Clients ({aggregation_technique})")
                ax_train.legend()
                st.pyplot(fig_train)

                # Create test scores plot
                fig_test, ax_test = plt.subplots(figsize=(10, 6))
                for idx, client in enumerate(clients):
                    ax_test.plot(client.test_scores, label=f'Client {idx + 1}')
                ax_test.set_xlabel('Epochs')
                ax_test.set_ylabel('Test Score')
                ax_test.set_title(f"Test Scores for All Clients ({aggregation_technique})")
                ax_test.legend()
                st.pyplot(fig_test)

                # Create combined plot with global model accuracy
                fig_combined, ax_combined = plt.subplots(figsize=(10, 6))
                for idx, client in enumerate(clients):
                    ax_combined.plot(client.test_scores, label=f'Client {idx + 1}')

                # Plot global model accuracy
                global_accuracies = [metrics['accuracy'] for metrics in global_metrics]
                ax_combined.plot(global_accuracies, label='Global Model Accuracy',
                               linestyle='--', color='black', linewidth=2)

                ax_combined.set_xlabel('Epochs')
                ax_combined.set_ylabel('Scores')
                ax_combined.set_title(f"Test Scores and Global Model Accuracy ({aggregation_technique})")
                ax_combined.legend()
                st.pyplot(fig_combined)

        status_text.text("Training Complete!")

        # Final metrics visualization (existing line chart)
